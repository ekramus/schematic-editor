...
public class SceneGraph implements Iterable<Node> {
	...
	/**
	 * This method generates <code>ArrayList</code> from this
	...
	/**
	 * This method creates static pre-defined <code>SceneGraph</code>. For
	...
	public void manualCreateSceneGraph3() {
		...
		// R vzor
		Rectangle rec = new Rectangle(new UnitPoint(0, 0),
				new UnitPoint(70, 20));
		...
		TransformationNode tArc = new TransformationNode(Transformation
				.getShift(25, 240));
		...
		// wiress
		ShapeNode l1 = new ShapeNode(new Line(new UnitPoint(50, 160),
				new UnitPoint(102, 160)));
		ShapeNode l2 = new ShapeNode(new Line(new UnitPoint(50, 160),
				new UnitPoint(50, 240)));
		ShapeNode l3 = new ShapeNode(new Line(new UnitPoint(50, 290),
				new UnitPoint(50, 375)));
		ShapeNode l4 = new ShapeNode(new Line(new UnitPoint(50, 375),
				new UnitPoint(240, 375)));
		ShapeNode l5 = new ShapeNode(new Line(new UnitPoint(240, 375),
				new UnitPoint(240, 340)));
		ShapeNode l6 = new ShapeNode(new Line(new UnitPoint(172, 160),
				new UnitPoint(240, 160)));
		ShapeNode l7 = new ShapeNode(new Line(new UnitPoint(240, 330),
				new UnitPoint(240, 270)));
		ShapeNode l8 = new ShapeNode(new Line(new UnitPoint(240, 200),
				new UnitPoint(240, 160)));
		ParameterNode pWires = new ParameterNode();
		pWires.setColor(Color.BLACK);
		pWires.setWidth(new Pixel(1));
		TransformationNode tWires = new TransformationNode(Transformation
				.getIdentity());

		ShapeNode text1 = new ShapeNode(new Text(new UnitPoint(125, 145), "R1",
				15));
		ShapeNode text2 = new ShapeNode(new Text(new UnitPoint(260, 230), "R2",
				15));
		ShapeNode text3 = new ShapeNode(new Text(new UnitPoint(270, 340), "C1",
				15));
		ShapeNode text4 = new ShapeNode(new Text(new UnitPoint(80, 260), "Uc",
				15));
		TransformationNode tText = new TransformationNode(Transformation
				.getIdentity());
	...
	/**
	 * This method creates static predefined working SceneGraph. For development
	 * purposes only.
	 */
	public void manualCreateSceneGraph2() {
		...
		ShapeNode sn1 = new ShapeNode(new Line(new UnitPoint(10, 10),
				new UnitPoint(200, 200)));
		ShapeNode sn2 = new ShapeNode(new Line(new UnitPoint(200, 10),
	...
	/**
	 * @deprecated This method returns list of Boundig Boxes of each group. It
	 *             should be not used, as it does not provide direct link to
	 *             Elements , which Bounding Box is linked to which element.
	 * @return
	 */
	@Deprecated
	...
	/**
	 * This method returns LinkedList of ElementNodes within given
	 * areaOfInterest.
	 * 
	 * @param areaOfInterest
	 *            rectangular area, in which are (possibly) some ElementNodes.
	 * @return LinkedList of GroupNodes, which were located at least partly
	 *         inside given areaOfInterest.
	 */
	public LinkedList<ElementNode> getElementNodeList(
			Rectangle2D.Double areaOfInterest) {
		...
		for (Node node : this) {
			if (node instanceof GroupNode) {
				...
				boundModifier = ((GroupNode) node).getChildrenParameterNode()
						.getWidth();
			}
			if (node instanceof ElementNode) {
				ElementNode en = (ElementNode) node;
				if (areaOfInterest.contains(en.getBounds(boundModifier))
						|| areaOfInterest.intersects(en
