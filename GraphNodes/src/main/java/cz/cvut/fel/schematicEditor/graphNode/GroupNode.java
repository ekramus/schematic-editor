...
public class GroupNode extends Node {
	...
	public Transformation getTransformation() {
		...
		for (Node child : childrenTransformationList) {
			t = Transformation.multiply(t, ((TransformationNode) child)
					.getTransformation());
		}
		return t;
	}
	...
	public boolean isHit(Rectangle2D rectangle) {
		...
		for (int i = getChildrenElementList().size() - 1; i >= 0; i--) {
			...
			if (child.isHit(getTransformation().shiftInverse(rectangle))) {
				logger.debug("element HIT: " + child + " transformation: "
						+ getTransformation());
				return true;
			}
		}
	...
	/**
	 * Getter for <code>NodeArray</code> with corrected
	 * <code>TransformationNode</code> and <code>ParameterNode</code>.
	 * 
	 * @param tn
	 *            transformation node.
	 * @param pn
	...
	public ArrayList<Node> getNodeArray(TransformationNode tn, ParameterNode pn) {
		...
		if (tn == null) {
			...
			t = new TransformationNode(Transformation.multiply(
					getTransformation(), tn.getTransformation()));
		}
		...
		for (ElementNode elementNode : childrenElementList) {
			if (!elementNode.isDisabled()) {
				if (elementNode.getElementType() == ElementType.T_PART) {
					...
					result.addAll(partNode.getPartGroupNode()
							.getNodeArray(t, p));
				} else {
	...
	public UnitRectangle getBounds() {
		...
		UnitRectangle bounds = getChildrenElementList().getFirst().getBounds(
				getChildrenParameterNode().getWidth());

		UnitRectangle result = new UnitRectangle(bounds.getX(), bounds.getY(),
	...
	/**
	 * Detects, whether given rectangle is in edit zone. If it is, it means,
	 * edit should be invoked.
	 * 
	 * @param r2d
	 *            rectangle around pointer.
	 * @return <code>true</code> if given rectangle contains any edit point from
	 *         group, else <code>false</code>.
	 */
	public boolean startEdit(Rectangle2D r2d) {
		...
		for (int i = getChildrenElementList().size() - 1; i >= 0; i--) {
			ElementNode child = getChildrenElementList().get(i);
			setEditedElement(child.startEdit(getTransformation().shiftInverse(
					r2d)));
			if (getEditedElement() != null) {
				logger.debug("element edit zone HIT: " + child
	...
	/**
	 * Detects, whether given rectangle is in rotate zone. If it is, it means,
	 * rotate should be invoked.
	 * 
	 * @param r2d
	 *            rectangle around pointer.
	 * @return <code>true</code> if given rectangle is in rotate point of whole
	 *         group, else <code>false</code>.
	 */
	...
	public Node duplicate() {
		...
		ParameterNode chPN = (ParameterNode) getChildrenParameterNode()
				.duplicate();
	 ...
	 */
	private void setChildrenElementList(
			LinkedList<ElementNode> childrenElementList) {
	...
	/**
	 * Calculates number of coordinates used for construction of
	 * {@link GroupNode}.
	 * 
	 * @return
	 */
	...
	public ElementModificator getElementModificator() {
		...
		for (ElementNode elementNode : getChildrenElementList()) {
			result = result
					.or(elementNode.getElement().getElementModificator());
		}
	...
	public ElementType getElementType() {
		...
		// Change it!
		return getChildrenElementList().getFirst().getElement()
				.getElementType();
	}
